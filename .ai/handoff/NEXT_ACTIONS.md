...
| Item | Resolution |
|------|------------|
| T-020: `slaComplianceRate` measured a resolution rate, not SLA compliance | Done: one definition, `slaVerdict` in `src/sla-engine.ts`, consumed by both `checkSlaCompliance` and `src/metrics.ts`. Rate is nullable, floored not rounded, and is 100 if and only if the engine reports zero breaches on the same decisions. `mttrCritical` removed. A cross-check suite runs both functions on one input, which no test did before. Evidence and the two deliberately open gaps are in STATUS.md; issue https://github.com/homeofe/supply-chain-guard/issues/172. Follow-up: T-021 / https://github.com/homeofe/supply-chain-guard/issues/194. |
