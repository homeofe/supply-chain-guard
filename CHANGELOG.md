...
- **`getBundledFeedRef()`, a read-only companion to `getBundledFeed()`.** It returns
  one stable frozen array for the whole process instead of a fresh copy per call,
  so a lookup index memoized on the feed array's identity stays valid. Exported
  from the package root, because an embedder that hands `getBundledFeed()` to a
  matcher hits the same defect this release fixes. `getBundledFeed()` is unchanged
  and still returns an independent, mutable array for reporting and export paths.
