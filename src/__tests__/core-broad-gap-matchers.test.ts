...
const parityCases: ParityCase[] = [
  ...
  { rule: "BEACON_TIMEOUT_FETCH", flags: "i", content: "axiosetTimeout(x fetch", expectedLines: [] },
  ...
  { rule: "IMPORT_EXPRESSION", flags: "g", content: "import( req.query.m)", expectedLines: [1], expectedStarts: [0], expectedEvidence: ["import( req.query"] },
  { rule: "IMPORT_EXPRESSION", flags: "g", content: "import(requested.m)", expectedLines: [] },
  ...
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "<script type=x>one</script> tail </script>", expectedLines: [1], expectedStarts: [0], expectedEvidence: ["<script"] },
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "<scripts x <script>y</script>", expectedLines: [1], expectedStarts: [11], expectedEvidence: ["<script"] },
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "<svg:script href=\"x\"/>", expectedLines: [1], expectedStarts: [0], expectedEvidence: ["<svg:script"] },
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "button onclick = \"run()\"", expectedLines: [1] },
  // A multi-line script is found on the line of its opening tag. The old
  // <script>...</script> form needed both tags on one line and missed this.
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "<script>\nrun()\n</script>", expectedLines: [1], expectedStarts: [0], expectedEvidence: ["<script"] },
  { rule: "SVG_SCRIPT_INJECTION", flags: "g", content: "<script-x/> <scripted> &lt;script&gt; </script >", expectedLines: [] },
  ...
  { rule: "VIDAR_WALLET_THEFT", flags: "g", content: "phantomjs seeding vaulted Exodus_seeds", expectedLines: [1], expectedStarts: [26], expectedEvidence: ["Exodus_seeds"] },
  { rule: "VIDAR_WALLET_THEFT", flags: "g", content: "phantomjs seeding vaulted", expectedLines: [] },
...
const nearMissUnits: Readonly<Record<string, string>> = {
  ...
  SVG_SCRIPT_INJECTION: "<scripts attr x ",
