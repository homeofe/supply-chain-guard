...
function formatMarkdown(report: ScanReport): string {
  ...
  lines.push(`|----------|-------|`);
  lines.push(`| Tool | \`${mdCell(report.tool || "supply-chain-guard v6.0.18")}\` |`);
...
function formatSbom(report: ScanReport): string {
  ...
  if (report.sbomDocument) {
    ...
    const annotations = buildIncidentAnnotations(
      ...
      subjectsByIncident,
      metadata.timestamp,
      metadata.tools.components[0]?.version ?? "6.0.18",
    );
  ...
  const sbom = {
    ...
    metadata: {
      ...
      tools: {
        components: [
          { type: "application", name: "supply-chain-guard", version: "6.0.18" },
        ],
      },
      component: {
...
function formatBadge(report: ScanReport): string {
  ...
  return JSON.stringify({
    ...
    message,
    color,
    tool: report.tool || "supply-chain-guard v6.0.18",
    version: report.tool ? (report.tool.includes("@") ? report.tool.split("@")[1] : report.tool.replace(/^supply-chain-guard v?/, "")) : "6.0.18",
    timestamp: report.timestamp,
...
function formatJunit(report: ScanReport): string {
  ...
  lines.push(
    `    <property name="supply-chain-guard:tool-version" value="${xmlEscape(report.tool || "supply-chain-guard v6.0.18")}"/>`,
  );
  lines.push(
...
<div class="container">
  <header>
    <h1>${escapeHtml(report.tool || "supply-chain-guard v6.0.18")} Scan Report</h1>
    <div class="meta">
  ...
  <footer>
    Generated by <a href="https://github.com/homeofe/supply-chain-guard">${escapeHtml(report.tool || "supply-chain-guard v6.0.18")}</a>
  </footer>
</div>
<script>
